                  ...
                  int nrhs, const mxArray *prhs[] )
{
    ...
    int		n_V	= mxGetM(prhs[0]);		/* Number of nodes */ //100
...
void bellman_ford_allpair(double *dis_mat, double *ang_mat,
						  ...
						  double *E, int n_E)
{
	const double INF_DIS	= n_V*n_E;//10000
	...
	// set viewable values
	double *pu	= E;//E为内距离点的编号，和内距离(出发点的编号)
	double *pv	= E+n_E;//第二个点的编号
	double *pw	= pv+n_E;//两个点间的距离
	for(e=0;e<n_E;e++)
	{
		u		= (int)pu[e]-1;
		v		= (int)pv[e]-1;
		...
		// set dis_mat
		MAT_SET(dis_mat,u,v,w,n_V);//dis_mat中存放w距离
		...
		dy	= Y[v]-Y[u];
		ang	= atan2(dy,dx);
		MAT_SET(ang_mat,u,v,ang,n_V);//ang_mat中存放ang
	...
	for(s=0;s<n_V;s++)
	{
		...
		/* Relaxation using standard bellman-ford*///松弛操作
		bstop	= false;//flag
