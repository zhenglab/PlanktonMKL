...
function [sc,V,E,dis_mat,ang_mat] = compu_contour_innerdist_SC(cont, fg_mask,n_dist, n_theta,bTangent, bSmoothCont, ifig)

if ~exist('bSmoothCont')	bSmoothCont = 0;	end
if ~exist('bTangent')		bTangent = 0;		end

%------ Parameters ----------------------------------------------
n_pt= size(cont,1);%100
X	= cont(:,1);
Y	= cont(:,2);
V	= cont;
...
E	= build_graph_contour_C(X,Y,fg_mask,bSmoothCont);%找到落在形状内部的
E	= E';
...
[dis_mat,ang_mat] = bellman_ford_ex_C(X,Y,E); %计算最短距离
...
if bTangent                      %计算内部角
	Xs	= [X(end); X; X(1)];
	Ys	= [Y(end); Y; Y(1)];
	gX	= gradient(Xs); %求数值函数梯度的指令
	gY	= gradient(Ys);

	thetas	= atan2(gY,gX);  %第四象限反正切
	thetas	= thetas(2:end-1);
	thetas	= repmat(thetas',n_pt,1);%平铺矩阵
