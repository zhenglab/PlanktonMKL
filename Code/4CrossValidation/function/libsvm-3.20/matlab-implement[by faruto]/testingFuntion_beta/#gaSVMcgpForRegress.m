function [bestCVmse,bestc,bestg,bestp,ga_option] = gaSVMcgpForRegress(train_label,train,ga_option)
% gaSVMcgForClass
% by faruto
...
if nargin == 2
    ga_option = struct('maxgen',100,'sizepop',20,'pCrossover',0.4,'pMutation',0.01, ...
                'cbound',[0.1,100],'gbound',[0.01,1000],'pbound',[0.01,100],'v',5);
end
...
p_len_chromosome = ceil(log2((ga_option.pbound(2)-ga_option.pbound(1))*100));
len_chromosome = c_len_chromosome+g_len_chromosome+p_len_chromosome;
...
for i = 1:ga_option.sizepop
    ...
    % 解码
    [c,g,p] = ga_decode(individuals.chromosome(i,:),ga_option.cbound,ga_option.gbound,ga_option.pbound);
    % 计算初始适应度(CV准确率)
    cmd = ['-v ',num2str(ga_option.v),' -c ',num2str( c ),' -g ',num2str( g ),' -p ',num2str( p ),' -s 3'];
...
for i=1:ga_option.maxgen
    ...
    for j = 1:ga_option.sizepop
        % 解码
        [c,g,p] = ga_decode(individuals.chromosome(j,:),ga_option.cbound,ga_option.gbound,ga_option.pbound);
        % 计算初始适应度
        cmd = ['-v ',num2str(ga_option.v),' -c ',num2str( c ),' -g ',num2str( g ),' -p ',num2str( p ),' -s 3'];
    ...
    [new_c,g,p] = ga_decode(new_bestchromosome,ga_option.cbound,ga_option.gbound,ga_option.pbound);
    [c,g,p] = ga_decode(bestchromosome,ga_option.cbound,ga_option.gbound,ga_option.pbound);
...
[bestc,bestg,bestp] = ga_decode(bestchromosome,ga_option.cbound,ga_option.gbound,ga_option.pbound);
...
function [c,g,p] = ga_decode(chromosome,cbound,gbound,pbound)
% ga_decode by faruto 
% Email:farutoliyang@gmail.com
% 2009.10.08
c_len_chromosome = ceil(log2((cbound(2)-cbound(1))*100));
g_len_chromosome = ceil(log2((gbound(2)-gbound(1))*100));
p_len_chromosome = ceil(log2((pbound(2)-pbound(1))*100));
len_chromosome = c_len_chromosome+g_len_chromosome+p_len_chromosome;

cdec = bin2dec( num2str(chromosome(1:c_len_chromosome)) );
gdec = bin2dec( num2str(chromosome(c_len_chromosome+1:c_len_chromosome+g_len_chromosome)) );
pdec = bin2dec( num2str(chromosome(c_len_chromosome+g_len_chromosome+1:end)) );

c = cbound(1) + cdec*(cbound(2)-cbound(1))/(2^(c_len_chromosome)-1);
g = gbound(1) + gdec*(gbound(2)-gbound(1))/(2^(g_len_chromosome)-1);
p = pbound(1) + pdec*(pbound(2)-pbound(1))/(2^(p_len_chromosome)-1);
