...
%   verbose    - be more verbose, also displays the filter bank
%
...
%   Jarmo Ilonen
%
% $Name:  $ $Id: sg_createfilterbank.m,v 1.6 2005/10/12 14:25:02 ilonen Exp $
...
if conf.user_freq==0 && conf.user_orientation==0
  ...
  % plot filters
  if conf.verbose>=1
    sg_plotfilters2(N,f,m,n,'k',conf.k,'p',conf.p,'gamma',conf.gamma,'eta',conf.eta);
  end;
else
  freq=sort(f);
  freq=freq(end:-1:1);
